...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel) and message.author.id != OWNER_ID:
        # Jika pesan adalah command (!...), proses command dulu — jangan intercept
        if message.content.startswith(bot.command_prefix):
            await bot.process_commands(message)
            return
        # Bukan command → cek apakah kirim foto verifikasi
        if message.attachments:
            ...
            return
        # Pesan DM biasa tanpa attachment dan bukan command
        await bot.process_commands(message)
        return
...
async def on_command_error(ctx, error):
    # Abaikan command yang tidak dikenal — tidak perlu balas
    if isinstance(error, commands.CommandNotFound):
        return
    # Sudah ada handler khusus di command itu sendiri? Lewati
    ...
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send(
            ...
            f"Ketik `!help` untuk panduan command.",
            delete_after=8
        )
    elif isinstance(error, commands.BadArgument):
        await ctx.send(
            "❌ Argumen tidak valid. Pastikan format sudah benar.",
            delete_after=8
        )
    elif isinstance(error, commands.CommandOnCooldown):
        await ctx.send(
            f"⏳ Cooldown! Coba lagi dalam **{error.retry_after:.1f}** detik.",
            delete_after=5
        )
    elif isinstance(error, commands.CheckFailure):
        await ctx.send("❌ Kamu tidak memenuhi syarat untuk command ini.", delete_after=5)
    else:
        # Log error tak terduga ke konsol, jangan biarkan diam
