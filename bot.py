...
async def help_cmd(ctx):
    ...
    embed.add_field(name="🎬 YouTube (Admin)",
    ...
    embed.add_field(name="⚠️ Warn",
        value="`!warn @user [alasan]`\n`!warnlist @user`\n`!clearwarn @user`", inline=False)
...
# State musik per guild
music_queues    = {}   # guild_id -> list of track dicts
music_current   = {}   # guild_id -> current track dict
music_volume    = {}   # guild_id -> int (0-100)
music_idle_task = {}   # guild_id -> asyncio Task
...
async def fetch_track(query: str) -> dict | None:
    """Ambil info satu lagu dari query/URL."""
    opts = YTDL_OPTS.copy()
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            if not info:
                print("[Music] fetch_track: info kosong")
                return None
            if "entries" in info:
                info = info["entries"][0]
            if not info:
                print("[Music] fetch_track: entry kosong")
                return None

            # Cari URL audio yang valid
            url = info.get("url")
            if not url:
                # Coba dari formats
            ...
            print(f"[Music] Resolved: {info.get('title')} | URL: {url[:60] if url else 'NONE'}")
            return {
        ...
        print(f"[Music] fetch_track error: {e}")
        return None

async def fetch_playlist(url: str, limit: int = 10) -> list:
    """Ambil daftar lagu dari playlist YouTube, maks `limit` lagu."""
    opts = {
        "format": "bestaudio/best",
        "noplaylist": False,
        "quiet": True,
        "no_warnings": True,
        "extract_flat": True,   # cepat, ambil metadata saja
        "playlistend": limit,
    }
    loop = asyncio.get_event_loop()
    try:
            ...
            for e in entries[:limit]:
                if e:
                    tracks.append({
                        ...
                        "url":      None,  # akan di-resolve saat diputar
...
async def play_next(guild: discord.Guild, text_channel: discord.TextChannel):
    """Putar lagu berikutnya di antrian."""
    gid   = guild.id
    queue = get_queue(gid)
    ...
    if not queue:
        # Mulai timer idle 3 menit lalu disconnect
        async def idle_disconnect():
            await asyncio.sleep(180)
            if vc and vc.is_connected():
                await vc.disconnect()
                try:
                    await text_channel.send("👋 Tidak ada lagu lagi. Bot keluar dari voice channel.")
                except Exception:
                    pass
            music_current.pop(gid, None)

        if gid in music_idle_task and not music_idle_task[gid].done():
            music_idle_task[gid].cancel()
        music_idle_task[gid] = asyncio.create_task(idle_disconnect())
        return

    track = queue.pop(0)

    # Resolve URL jika belum ada (dari flat playlist extract)
    if not track.get("url") and track.get("webpage"):
        resolved = await fetch_track(track["webpage"])
        if resolved:
            track = resolved

    if not track.get("url"):
        await text_channel.send(f"⚠️ Gagal load: **{track['title']}**, skip...")
        await play_next(guild, text_channel)
        return

    music_current[gid] = track
    vol = music_volume.get(gid, 80) / 100

    try:
        source = discord.FFmpegPCMAudio(track["url"], **FFMPEG_OPTS)
        source = discord.PCMVolumeTransformer(source, volume=vol)
    except Exception as e:
        print(f"[Music] FFmpeg source error: {e}")
        await text_channel.send(f"❌ Gagal load audio: `{e}`. Skip ke lagu berikutnya...")
        await play_next(guild, text_channel)
        return
    ...
    def after_play(error):
        ...
        fut = asyncio.run_coroutine_threadsafe(play_next(guild, text_channel), bot.loop)
        try:
            fut.result()
        except Exception as e:
    ...
    try:
        ...
        print(f"[Music] vc.play error: {e}")
        await text_channel.send(f"❌ Error saat memutar: `{e}`")
        return
    ...
    embed.add_field(name="⏱ Durasi",    value=fmt_duration(track["duration"]), inline=True)
    embed.add_field(name="👤 Channel",  value=track["uploader"],               inline=True)
    embed.add_field(name="📋 Antrian",  value=f"{len(get_queue(gid))} lagu",   inline=True)
    if track.get("thumbnail"):
        embed.set_thumbnail(url=track["thumbnail"])
    embed.set_footer(text="Asisten Lurah BFL • Music Player")
    try:
...
# ── Playlist navigation view ──
class PlaylistView(View):
    def __init__(self, url: str, guild_id: int, text_channel, offset: int = 0):
    ...
    @discord.ui.button(label="▶ Load 10 Lagu Berikutnya", style=discord.ButtonStyle.primary,
                       emoji="⏭")
    async def next_batch(self, interaction: discord.Interaction, button: Button):
        await interaction.response.defer()
        new_offset = self.offset + 10
        opts = {
            "format": "bestaudio/best",
            "noplaylist": False,
            "quiet": True,
            "no_warnings": True,
            "extract_flat": True,
            "playliststart": new_offset + 1,
            "playlistend":   new_offset + 10,
        }
        loop = asyncio.get_event_loop()
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                info    = await loop.run_in_executor(None, lambda: ydl.extract_info(self.url, download=False))
                entries = info.get("entries", [])
        except Exception:
            entries = []

        if not entries:
            await interaction.followup.send("⚠️ Tidak ada lagu berikutnya di playlist.", ephemeral=True)
            return
        ...
        embed = discord.Embed(
            ...
            description=f"✅ **{added}** lagu ditambahkan ke antrian (lagu #{new_offset+1}–{new_offset+added}).",
            color=discord.Color.green()
        )
        embed.set_footer(text="Asisten Lurah BFL • Music Player")
        new_view = PlaylistView(self.url, self.guild_id, self.text_channel, new_offset)
...
async def play(ctx, *, query: str):
    if not ctx.author.voice:
        await ctx.send("❌ Kamu harus berada di voice channel dulu!", delete_after=10)
        return
    ...
    if vc and vc.channel != ctx.author.voice.channel:
        ...
        # Retry connect sampai 3x karena Railway sering timeout ke Discord voice
        for attempt in range(3):
            try:
                vc = await ctx.author.voice.channel.connect(timeout=30.0, reconnect=True)
                break
            except Exception as e:
                print(f"[Music] Connect attempt {attempt+1} failed: {e}")
                if attempt == 2:
                    await ctx.send(f"❌ Gagal terhubung ke voice channel setelah 3 percobaan. Coba lagi.")
                    return
                await asyncio.sleep(2)

    # Cancel idle timer kalau ada
    gid = ctx.guild.id
    if gid in music_idle_task and not music_idle_task[gid].done():
        music_idle_task[gid].cancel()

    msg = await ctx.send("🔍 Mencari lagu...")

    # Cek apakah playlist
    is_playlist = "playlist" in query.lower() or ("youtube.com/playlist" in query)

    if is_playlist:
        tracks = await fetch_playlist(query, limit=10)
        if not tracks:
            await msg.edit(content="❌ Gagal load playlist.")
            return
        queue = get_queue(gid)
        for t in tracks:
            queue.append(t)
        ...
        embed = discord.Embed(
            title="📋 Playlist Dimuat",
            description=f"✅ **{len(tracks)}** lagu pertama ditambahkan ke antrian.",
            color=discord.Color.blue()
        )
        embed.set_footer(text="Klik tombol di bawah untuk load 10 lagu berikutnya")
        await msg.edit(content=None, embed=embed,
                       view=PlaylistView(query, gid, ctx.channel))
    else:
        track = await fetch_track(query)
        if not track:
            await msg.edit(content="❌ Lagu tidak ditemukan.")
            return
        queue = get_queue(gid)
...
async def skip(ctx):
    ...
    if not vc or not vc.is_playing():
        await ctx.send("❌ Tidak ada lagu yang diputar.", delete_after=5)
        return
    vc.stop()
    await ctx.send("⏭ Lagu di-skip!", delete_after=5)

@bot.command(name="stop")
async def stop(ctx):
    vc = ctx.guild.voice_client
    if not vc:
        await ctx.send("❌ Bot tidak ada di voice channel.", delete_after=5)
        return
    gid = ctx.guild.id
    music_queues[gid] = []
    music_current.pop(gid, None)
    ...
    vc.stop()
    await vc.disconnect()
    await ctx.send("⏹ Musik dihentikan dan bot keluar dari voice channel.")
...
async def pause(ctx):
    ...
    if vc and vc.is_playing():
        vc.pause()
        await ctx.send("⏸ Musik di-pause.", delete_after=5)
    else:
        await ctx.send("❌ Tidak ada lagu yang diputar.", delete_after=5)
...
async def resume(ctx):
    ...
    if vc and vc.is_paused():
        vc.resume()
        await ctx.send("▶ Musik dilanjutkan.", delete_after=5)
    else:
        await ctx.send("❌ Musik tidak sedang di-pause.", delete_after=5)
...
async def queue_cmd(ctx):
    ...
    if not current and not queue:
        await ctx.send("📭 Antrian kosong.", delete_after=8)
        return

    embed = discord.Embed(title="🎵 Antrian Musik — BFL", color=discord.Color.purple())

    if current:
        embed.add_field(
            name="▶ Sekarang Diputar",
            value=f"**{current['title']}** `{fmt_duration(current['duration'])}`",
            inline=False
        )
    ...
    if queue:
        ...
        if len(queue) > 10:
            lines.append(f"... dan **{len(queue)-10}** lagu lagi")
...
async def nowplaying(ctx):
    ...
    current = music_current.get(gid)
    if not current:
        await ctx.send("❌ Tidak ada lagu yang diputar.", delete_after=8)
        return
    embed = discord.Embed(
        title="🎵 Sekarang Diputar",
    ...
    embed.add_field(name="⏱ Durasi",   value=fmt_duration(current["duration"]), inline=True)
    embed.add_field(name="👤 Channel", value=current["uploader"],               inline=True)
    embed.add_field(name="🔊 Volume",  value=f"{music_volume.get(gid, 80)}%",   inline=True)
...
async def volume(ctx, vol: int):
    if not 1 <= vol <= 100:
        await ctx.send("❌ Volume harus antara 1-100.", delete_after=5)
        return
    gid = ctx.guild.id
    ...
    await ctx.send(f"🔊 Volume diset ke **{vol}%**", delete_after=5)
...
async def shuffle(ctx):
    ...
    queue = get_queue(gid)
    if not queue:
        await ctx.send("❌ Antrian kosong.", delete_after=5)
        return
    random.shuffle(queue)
    await ctx.send(f"🔀 Antrian diacak! ({len(queue)} lagu)", delete_after=5)

@bot.command(name="remove")
async def remove(ctx, nomor: int):
    gid   = ctx.guild.id
    queue = get_queue(gid)
    if not queue or nomor < 1 or nomor > len(queue):
        await ctx.send("❌ Nomor tidak valid.", delete_after=5)
        return
    removed = queue.pop(nomor - 1)
    await ctx.send(f"🗑 **{removed['title']}** dihapus dari antrian.", delete_after=8)

@bot.command(name="clearqueue", aliases=["cq"])
async def clear_queue(ctx):
    gid = ctx.guild.id
    music_queues[gid] = []
    await ctx.send("🗑 Antrian dikosongkan.", delete_after=5)
