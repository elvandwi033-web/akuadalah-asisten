...
BONANZA_WEIGHTS  = [35,    32,    28,   25,     22,    18,    8,      1]
# Total bobot = 169. Lolipop = 1/169 per sel ≈ 0.59%
# P(≥3 lolipop di 25 sel) ≈ 0.08%  →  sangat langka

# ── Tabel hadiah per simbol (satuan per 1 Bcash bet, flat — bukan multiplier) ──
# Threshold MINIMUM cluster dinaikkan ke 10 (dulu 8) → lebih susah dapat hadiah
# Contoh: 🍇 10× di grid dengan bet 5 → 5 × 1 = 5 Bcash
# Untuk mencapai 10 simbol sama di 25 sel:
#   - Anggur (bobot 35/169 ≈ 20.7%) → E(anggur per grid) ≈ 5.2 → butuh RNG bagus
#   - Berlian (bobot 8/169 ≈ 4.7%)  → E(berlian per grid) ≈ 1.2 → hampir mustahil natural
BONANZA_PAYOUTS = {
    # Simbol murah — threshold 10, hadiah kecil
    "🍇": {10: 1,  12: 2,  15: 3,  18: 5,  22: 8,  25: 13},
    "🍊": {10: 1,  12: 2,  15: 4,  18: 6,  22: 10, 25: 16},
    "🍋": {10: 2,  12: 3,  15: 5,  18: 8,  22: 13, 25: 20},
    # Simbol sedang — threshold 10, hadiah lebih besar
    "🍉": {10: 2,  12: 4,  15: 7,  18: 11, 22: 17, 25: 25},
    "🍑": {10: 3,  12: 5,  15: 9,  18: 14, 22: 22, 25: 35},
    # Simbol mahal — threshold 10, hadiah premium
    "🍒": {10: 5,  12: 9,  15: 15, 18: 24, 22: 38, 25: 60},
    "💎": {10: 10, 12: 18, 15: 30, 18: 50, 22: 80, 25: 120},
    # 💎 sangat langka (bobot 8) — kalau muncul 10× itu luar biasa
}

# ── Tabel multiplier KHUSUS FREE SPIN ─────────────────
# Multiplier hanya aktif saat free spin, bukan putaran normal
# 25% dapat besar (×3–×15), 75% dapat kecil (×1.5–×3)
# Dulu 40/60 → diperketat jadi 25/75 → susah dapat big mult
BONANZA_FS_MULT_BIG   = (3.0, 15.0)   # 25% → range multiplier besar
BONANZA_FS_MULT_SMALL = (1.5,  3.0)   # 75% → range multiplier kecil
...
def _bz_calculate_win_freespin(grid, bet: int):
    ...
    Hadiah = flat Bcash × multiplier acak (40% besar x2–x15, 60% kecil x2–x3).
    """
    total_win  = 0
    win_details = []
    for sym in BONANZA_SYMBOLS:
        ...
        if count >= 8:
            ...
            if payout_rate > 0:
                ...
                # Tentukan tier multiplier: 25% besar, 75% kecil
    ...
    if not bet or bet < BONANZA_MIN_BET:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• **10+ simbol sama** di grid → cluster WIN!\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• Lolipop **sangat langka** — jaga harapanmu! 🙏\n"
