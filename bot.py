...
def _bz_calculate_win_freespin(grid, bet: int):
    """
    Hitung kemenangan free spin — sistem 3 tipe:
    - 30% jackpot : multiplier ×8–×20 dari cluster (atau flat ×2 jika tidak ada cluster)
    - 30% flat    : dapat tepat ×2 dari jumlah bet (tidak peduli cluster)
    - 40% normal  : multiplier ×3–×8 dari cluster (atau flat ×2 jika tidak ada cluster)
    """
    total_win  = 0
    win_details = []
    ...
    if roll < 0.30:
        ...
        # 30% flat ×2 — tidak peduli cluster
        win = bet * 2
        win_details.append(("🎲", 0, 0, 2.0, win, False))
        return win, win_details
    else:
        spin_type = "normal"
    ...
    # Tidak ada cluster → fallback flat ×2
...
async def bonanza_cmd(ctx, bet: int = None):
    ...
    async def _run_spin(is_fs: bool, ante: bool):
        ...
        if lolipop_count >= 3 and not is_fs and free_spins_left == 0 and total_spin_global >= scatter_target:
