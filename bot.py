...
async def help_admin_cmd(ctx):
    ...
    embed.add_field(name="💰 Bcash & Market Admin",
        value=("`!givecoin @user <jumlah>` *(DM/server)* — Tambah Bcash ke user\n"
               ...
               "📌 WD minimum **Rp50.000** | Semua kekalahan game → masuk saldo admin"), inline=False)
    ...
    embed.add_field(name="🎰 Lotre",
        value=("`!setlotre <hadiah>` — Buka lotre & set hadiah\n"
               "`!drawlotre` / `!undi` — Undi pemenang lotre"), inline=False)
...
async def delete_market_item(ctx, item_id: str = None):
    """Admin hapus item dari market."""
    if not is_admin(ctx.author):
        await ctx.send("❌ Hanya admin yang bisa menghapus item.", delete_after=5)
        return
    if not item_id:
...
async def list_user_coins(ctx):
    """Hanya admin — tampilkan semua user yang punya lebih dari 1 Bcash."""
    if not is_admin(ctx.author):
        await ctx.send("❌ Hanya admin yang bisa melihat command ini.", delete_after=5)
        return
...
MINING_LEVELS = {    1: {"cost":10,"hours":10,"reward":15},
...
async def trade_cmd(ctx, direction:str=None, amount:int=None):
    ...
    if result_up == pick_up:
        ...
        # 🪙 Trade LOSE: bet masuk admin + burn → supply turun → bullish
...
async def lotre_cmd(ctx, jumlah: int = 1):
    ...
    add_coins(ADMIN_BANK_ID, +total_cost)   # tiket lotre masuk ke pot admin
    burn_coins(total_cost)                   # burn tetap untuk pengaruhi harga
    log_history(uid, f"🎫 Beli {jumlah} tiket lotre", -total_cost)
    log_history(ADMIN_BANK_ID, f"🎫 Pot lotre dari {ctx.author.display_name} ({jumlah} tiket)", +total_cost)
