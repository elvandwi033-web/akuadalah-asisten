    ...
    while game_active:
        ...
        if auto_gaib_mode:
            ...
            if bal_now < bet:
                auto_gaib_mode = False
                embed_game.clear_fields()
                embed_game.add_field(name="❌ Bcash Habis", value=f"Auto Gaib berhenti. Saldo tidak cukup.", inline=False)
                await game_msg.edit(embed=embed_game)
                game_active = False
                break
            ...
            # Jalankan spin TANPA animasi (langsung hitung hasil)
            scatter_hit = await _run_spin(is_fs=False, ante=False)

            # Update embed ringkas (tanpa animasi grid penuh)
            ...
            stop_view = BonanzaAutoStopView(uid)
            await game_msg.edit(embed=embed_game, view=stop_view)

            if scatter_hit:
                await asyncio.sleep(1.0)
            else:
                # Tunggu sebentar, cek jika user tekan stop
                await asyncio.sleep(0.3)
                if stop_view.stop_signal:
                    auto_gaib_mode = False
                    embed_game.title = "🍬 SWEET BONANZA"
                    embed_game.add_field(name="⏹️ Auto Gaib Dihentikan", value=f"Total: **{total_won:,} Bcash**", inline=False)
                    view = BonanzaView(uid, bet, free_spins_left=0, ante_mode=ante_mode)
                    await game_msg.edit(embed=embed_game, view=view)
                    await view.wait()
                    choice = view.choice or "stop"
                    if choice == "stop":
                        ...
                        # Reset choice ke auto/next sesuai pilihan
                        if choice.startswith("auto"):
                            pass  # akan dihandle di loop berikutnya
                    continue
            continue
        ...
        if auto_spins_left > 0:
            ...
            # Tampil stop button selama auto
            stop_view = BonanzaAutoStopView(uid)
            await game_msg.edit(embed=embed_game, view=stop_view)
            ...
            if auto_spins_left <= 0 and not is_free_spin:
                ...
                if choice == "stop":
                    game_active = False
                elif choice == "auto10":  auto_spins_left = 10
                elif choice == "auto30":  auto_spins_left = 30
                elif choice == "auto50":  auto_spins_left = 50
        ...
        # Auto spin choices
        if choice == "auto10":   auto_spins_left = 10;  continue
        if choice == "auto30":   auto_spins_left = 30;  continue
        if choice == "auto50":   auto_spins_left = 50;  continue
