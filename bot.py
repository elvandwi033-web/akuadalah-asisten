...
async def help_cmd(ctx):
    ...
    embed.add_field(name="💰 Bcash System (di #spam)",
        value=("`!koin [@user]` — Cek saldo + nilai IDR Bcash kamu\n"
               ...
               "`!convertcoin <jumlah>` — Tukar Bcash ke IDR (min. 250), tarik ke DANA/OVO/ShopeePay\n"
...
async def help_admin_cmd(ctx):
    ...
    embed.add_field(name="💰 Bcash & Market Admin",
        value=("`!givecoin @user <jumlah>` — Tambah Bcash ke user (supply naik → harga ↓)\n"
               ...
               "`!supply` — **Laporan sirkulasi**: beredar holder, nama akun, coin di-burn\n"
...
async def supply_cmd(ctx):
    """Admin — lihat info sirkulasi: total supply beredar, daftar holder, total burn."""
    if not is_admin(ctx.author):
        await ctx.send("❌ Hanya admin yang bisa melihat command ini.", delete_after=5)
        return
...
async def convert_coin(ctx, amount: int = None):
    if not is_game_channel(ctx):
        return await ctx.send(game_channel_msg(ctx), delete_after=8)
    if not amount or amount < 250:
        return await ctx.send("❌ Minimum convert **250 Bcash**. Contoh: `!convertcoin 250`")
    uid = str(ctx.author.id)
    ensure_coins(uid)
    bal = get_coins(uid)
    if bal < amount:
    ...
    log_history(uid, f"💱 Convert ke IDR (Rp{idr:,})", -amount)

    await ctx.send(f"✅ Convert **{amount:,} Bcash** berhasil. Cek DM kamu untuk isi data penarikan!")
