...
async def on_ready():
    ...
    if ADMIN_BANK_ID not in coins:
        coins[ADMIN_BANK_ID] = 10000
        save_coins(coins)
        print(f"✅ Saldo admin bank diinisialisasi: 10,000 Bcash")
...
_PRICE_FLOOR   = 1000    # harga minimum absolut
_PRICE_CAP     = 50000   # harga maksimum absolut
_SUPPLY_INIT   = 10000   # supply awal
_PRICE_FILE_DEFAULT = {
    "price":         _PRICE_INITIAL,
    "supply":        _SUPPLY_INIT,
    "history":       [],
    # Pressure: akumulasi tekanan dari aktivitas game
...
# ── Fungsi trigger harga — dipanggil dari game ────────
def price_trigger(pressure_delta: float, reason: str = ""):
    """
    Tambahkan tekanan ke antrian harga.
    pressure_delta > 0  → tekanan naik
    pressure_delta < 0  → tekanan turun
    Tekanan ini akan diserap saat auto-tick 2 menit.
    """
    data = load_price()
    data["pressure"] = data.get("pressure", 0.0) + pressure_delta
    save_price(data)
    if reason:
        print(f"[PRICE TRIGGER] {reason} | delta={pressure_delta:+.2f} | accum={data['pressure']:+.2f}")

# ── Inti: hitung harga baru ───────────────────────────
def _calc_new_price(current: int, supply: int, pressure: float) -> int:
    """
    Faktor penentu harga (mirip crypto):
    1. Random walk kecil (±0.8%)
    2. Supply pressure — supply naik → harga turun, supply turun → harga naik
    3. Accumulated game pressure dari aktivitas user
    4. Mean-reversion lemah ke harga awal agar tidak drift jauh
    """
    # 1. Random walk baseline
    rand_move    = random.uniform(-0.008, 0.008) * current

    # 2. Supply pressure — supply_ref = kondisi netral
    supply_ref   = _SUPPLY_INIT
    supply_ratio = (supply - supply_ref) / max(supply_ref, 1)
    supply_move  = -supply_ratio * 60      # tiap 1% excess supply → -6 IDR

    # 3. Game pressure (sudah terakumulasi)
    # pressure dalam unit IDR langsung; dikonsumsi 50% per tick
    pressure_move = pressure * 0.5

    # 4. Mean-reversion ringan ke harga awal
    mid          = _PRICE_INITIAL
    revert_move  = (mid - current) * 0.002

    raw = current + rand_move + supply_move + pressure_move + revert_move
    return max(_PRICE_FLOOR, min(_PRICE_CAP, int(round(raw))))

# ── Auto tick setiap 2 menit ──────────────────────────
def market_tick() -> int:
    """Auto-tick setiap 2 menit. Konsumsi pressure dari aktivitas game."""
    data     = load_price()
    pressure = data.get("pressure", 0.0)
    new_price = _calc_new_price(data["price"], data["supply"], pressure)
    data["price"]    = new_price
    # Pressure dikurangi 50% (sisanya carry over)
    data["pressure"] = pressure * 0.5
    hist = data.get("history", [])
    hist.append(new_price)
    data["history"] = hist[-144:]   # simpan 144 titik = 4.8 jam
    save_price(data)
    return new_price
...
async def setprice(ctx, new_price: int = None):
    ...
    if not new_price or new_price < _PRICE_FLOOR:
        return await ctx.send(f"❌ Contoh: `!setprice 10000` (min Rp{_PRICE_FLOOR:,})")
    data = load_price()
    new_price      = min(new_price, _PRICE_CAP)
    data["price"]  = new_price
    data["pressure"] = 0.0   # reset pressure saat admin override
    save_price(data)
    await ctx.send(f"✅ Harga Bcash diset ke **Rp{new_price:,}** / 100 Bcash. Pressure direset.")
...
async def price_cmd(ctx):
    ...
    # Trend: bandingkan dengan 6 tick lalu (12 menit)
    if len(hist) >= 7:
        old = hist[-7]
        change_pct = ((price - old) / old) * 100
        if price > old:
            trend = f"📈 Naik ({change_pct:+.2f}%)"
            trend_color = discord.Color.green()
        elif price < old:
            trend = f"📉 Turun ({change_pct:+.2f}%)"
    ...
    # Pressure indicator
    if pressure > 50:
        pressure_str = "🟢 Bullish kuat"
    elif pressure > 10:
        pressure_str = "🟡 Bullish lemah"
    elif pressure < -50:
        pressure_str = "🔴 Bearish kuat"
    elif pressure < -10:
        pressure_str = "🟠 Bearish lemah"
    else:
        pressure_str = "⚪ Netral"

    # All-time high/low dari history
    all_prices = hist + [price]
    ath = max(all_prices)
    atl = min(all_prices)

    embed = discord.Embed(title="💹 Bcash Market — Live Price", color=trend_color)
    embed.add_field(name="💰 Harga Sekarang",    value=f"**Rp{price:,}** / 100 Bcash",    inline=True)
    embed.add_field(name="📦 Total Supply",       value=f"**{supply:,}** Bcash",            inline=True)
    embed.add_field(name="📊 Tren 12 Menit",     value=trend,                               inline=True)
    embed.add_field(name="🔋 Market Sentiment",  value=pressure_str,                        inline=True)
    embed.add_field(name="📈 ATH (sesi ini)",    value=f"Rp{ath:,}",                       inline=True)
    embed.add_field(name="📉 ATL (sesi ini)",    value=f"Rp{atl:,}",                       inline=True)
    embed.add_field(name="📉 Grafik",            value=f"`{spark}`",                        inline=False)
    embed.add_field(
        name="ℹ️ Pemicu Harga",
        value=(
            "🎰 **Slot kalah** → Bcash burn → Harga naik\n"
            "🎰 **Slot menang** → Bcash beredar → Harga turun\n"
            "📈 **Trade lose** → Bcash burn → Harga naik\n"
            "📈 **Trade win** → Bcash beredar → Harga turun\n"
            "⚔️ **Duel** → Kedua bet burn → Harga naik\n"
            "💝 **Admin givecoin** → Supply naik → Harga turun\n"
            "💱 **Convert/Buy** → Bcash burn → Harga naik"
        ),
        inline=False
    )
    embed.set_footer(text="Harga Bcash bergerak tiap 2 menit | Dipengaruhi aktivitas game & sirkulasi")
    await ctx.send(embed=embed)
