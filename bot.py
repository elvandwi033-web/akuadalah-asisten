...
async def setgame_cmd(ctx, action: str = None, game: str = None, value: str = None):
    ...
    if not action or action.lower() == "info":
        ...
        embed.set_footer(text="Gunakan !setgame winrate/maxbet/reward/adminwinrate untuk edit")
    ...
    if action == "winrate":
        ...
        # !setgame huntreward — reset hunt reward tiers via step-by-step
        await ctx.send(
        ...
        await ctx.send(
            "❌ Action tidak valid.\n"
            "Pilih: `winrate` | `adminwinrate` | `maxbet` | `reward` | `info`"
        )
...
# ── Tabel hadiah per simbol — threshold 8 seperti Pragmatic asli ──
# Cluster minimum 8 simbol (sesuai Sweet Bonanza asli)
# Contoh: 🍇 8× di grid dengan bet 5 → 5 × 1 = 5 Bcash
BONANZA_PAYOUTS = {
    # Simbol murah — threshold 8, hadiah lebih besar
    "🍇": {8: 1,  10: 2,  12: 3,  15: 5,  18: 8,  22: 13, 25: 20},
    "🍊": {8: 1,  10: 2,  12: 4,  15: 6,  18: 10, 22: 16, 25: 25},
    "🍋": {8: 2,  10: 3,  12: 5,  15: 8,  18: 13, 22: 20, 25: 30},
    # Simbol sedang — threshold 8, hadiah lebih besar
    "🍉": {8: 2,  10: 4,  12: 7,  15: 11, 18: 17, 22: 25, 25: 40},
    "🍑": {8: 3,  10: 5,  12: 9,  15: 14, 18: 22, 22: 35, 25: 55},
    # Simbol mahal — threshold 8, hadiah premium
    "🍒": {8: 5,  10: 9,  12: 15, 15: 24, 18: 38, 22: 60, 25: 90},
    "💎": {8: 10, 10: 18, 12: 30, 15: 50, 18: 80, 22: 120, 25: 180},
}

# ── Tabel multiplier KHUSUS FREE SPIN — 60% jackpot chance seperti Pragmatic ──
# 60% dapat besar (×3–×100), 40% dapat kecil (×2–×5)
BONANZA_FS_MULT_BIG   = (3.0, 100.0)  # 60% → range multiplier besar (bisa x100!)
BONANZA_FS_MULT_SMALL = (2.0,   5.0)  # 40% → range multiplier kecil
...
def _bz_calculate_win_normal(grid, bet: int):
    ...
    Threshold cluster minimum: 8 (sesuai Pragmatic Sweet Bonanza asli).
...
def _bz_force_freespin_grid():
    """
    Generate grid yang DIJAMIN menghasilkan minimal 1 cluster 8+.
    Seperti Pragmatic: free spin SELALU ada kemenangan.
    """
    best_grid = None
    best_score = -1
    # Coba hingga 30 kali, pilih grid dengan cluster terbaik
    for _ in range(30):
        grid = []
        for _ in range(5):
            row = random.choices(BONANZA_SYMBOLS[:-1], weights=BONANZA_WEIGHTS[:-1], k=5)
            grid.append(row)
        # Hitung total simbol teratas
        counts = {sym: _bz_count_symbol(grid, sym) for sym in BONANZA_SYMBOLS[:-1]}
        max_count = max(counts.values())
        if max_count >= 8:
            # Grid bagus, langsung pakai
            return grid
        if max_count > best_score:
            best_score = max_count
            best_grid = grid
    # Fallback: paksa inject simbol dominan
    dominant_sym = random.choices(
        ["🍇", "🍊", "🍋", "🍉", "🍑", "🍒"],
        weights=[50, 46, 42, 38, 34, 28], k=1
    )[0]
    # Paksa 8 simbol dominan di posisi acak
    positions = [(r, c) for r in range(5) for c in range(5)]
    random.shuffle(positions)
    for i in range(8):
        r, c = positions[i]
        best_grid[r][c] = dominant_sym
    return best_grid

def _bz_calculate_win_freespin(grid, bet: int):
    """
    Hitung kemenangan free spin.
    Hadiah = flat Bcash × multiplier acak (60% besar ×3–×100, 40% kecil ×2–×5).
    Sesuai Pragmatic Sweet Bonanza: free spin SELALU rewarding!
    """
    total_win  = 0
    win_details = []
    for sym in BONANZA_SYMBOLS:
        ...
        if count >= 8:
            ...
            if payout_rate > 0:
                ...
                # 60% dapat multiplier besar (×3–×100), 40% kecil (×2–×5)
                if random.random() < 0.60:
                    mult = round(random.uniform(*BONANZA_FS_MULT_BIG), 1)
                    is_big = True
                else:
                    mult = round(random.uniform(*BONANZA_FS_MULT_SMALL), 1)
                    is_big = False
...
class BonanzaView(View):
    def __init__(self, uid: str, bet: int, free_spins_left: int = 0, ante_mode: bool = False):
        super().__init__(timeout=90)
        self.uid             = uid
        self.bet             = bet
        self.free_spins_left = free_spins_left
        self.ante_mode       = ante_mode
        self.choice          = None   # "next" | "stop" | "ante" | "buyfs"
    ...
    @discord.ui.button(label="🎁 Buy Free Spin (×100 bet)", style=discord.ButtonStyle.secondary, row=1)
    ...
    @discord.ui.button(label="⏹️ Stop & Klaim", style=discord.ButtonStyle.danger, row=1)
...
async def bonanza_cmd(ctx, bet: int = None):
    ...
    if not bet or bet < BONANZA_MIN_BET:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• **8+ simbol sama** di grid → cluster WIN! (seperti Pragmatic asli)\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• Lolipop lebih sering muncul dari sebelumnya! 🙌\n"
                "• Free Spin: hadiah flat × **multiplier acak**\n"
                "  ▸ **60%** dapat **×3.0 – ×100.0** 🔥🔥 JACKPOT!\n"
                "  ▸ 40% dapat **×2.0 – ×5.0**\n"
                "• Free Spin **DIJAMIN ADA WIN** setiap putaran!\n"
    ...
    async def _run_spin(is_fs: bool, ante: bool):
        nonlocal total_won, tumble_count
        ...
        # ── Scatter check (hanya di spin normal, bukan saat sudah di free spin) ──
        ...
        if lolipop_count >= 3 and not is_fs and free_spins_left == 0:
            ...
            await game_msg.edit(embed=embed_game)
            await asyncio.sleep(1.2)  # ⚡ dari 2.0 → 1.2
    ...
    while game_active:
        ...
        if is_free_spin and free_spins_left > 0:
            ...
            if free_spins_left <= 0:
                ...
                embed_game.add_field(
                    ...
                    value=f"10 Free Spin habis. Kamu bisa lanjut spin atau berhenti.",
                    inline=False
                )
                await game_msg.edit(embed=embed_game)

                # Setelah free spin selesai → tombol normal
                view = BonanzaView(uid, bet, free_spins_left=0, ante_mode=ante_mode)
                await game_msg.edit(embed=embed_game, view=view)
                await view.wait()
                choice = view.choice or "stop"
            else:
                # Masih ada free spin — tampilkan tombol FS
            ...
            if choice == "stop":
                game_active = False
            # "next" → loop lanjut (is_free_spin masih True jika ada sisa)
            continue

        # ═══ MODE NORMAL ═══
        # Tampilkan tombol pilihan
        view = BonanzaView(uid, bet, free_spins_left, ante_mode)
        await game_msg.edit(embed=embed_game, view=view)
        await view.wait()
        choice = view.choice or "stop"
        ...
        if choice == "buyfs":
            ...
            embed_game.color = discord.Color.gold()
            await game_msg.edit(embed=embed_game, view=None)
            await asyncio.sleep(0.8)  # ⚡ dari 1.5 → 0.8
            continue
