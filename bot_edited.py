...
def _plinko_board_art(mult: float) -> str:
    ...
    slots = ["×0", "×0💀ZONK", "×1.25", "×3", "×10", "×15"]
    # Tandai slot yang aktif — dua slot ×0, keduanya highlight jika mult==0
    if mult == 0.0:
        active_indices = [0, 1]
    elif mult == 1.25:
        active_indices = [2]
        ...
        active_indices = []
    slot_row = " | ".join(
        f"[{s}]" if i in active_indices else f" {s} "
    ...
    if not bet or bet < PLINKO_MIN_BET:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "💣 **×0 ZONK** — 2% (zonk ekstra, bet di-burn)"
            ),
            inline=False
        )
...
async def mines_cmd(ctx, bet: int = None, difficulty: str = None):
    ...
    if bet is None:
        ...
        embed.add_field(
            ...
            value=(
                "🟢 `easy`   — **5 bom** (aman, multiplier kecil)\n"
                "🟡 `medium` — **7 bom** (standar) ← default\n"
                "🔴 `hard`   — **12 bom** (risiko tinggi)\n"
                "☠️ `expert` — **17 bom** (sangat riskan)\n"
    ...
    # ── Parse kesulitan ──
    difficulty_map = {
        "easy": 5, "mudah": 5, "medium": 7, "sedang": 7, "normal": 7,
        "hard": 12, "sulit": 12, "expert": 17, "extreme": 17,
    }
    ...
    diff_label = {5: "🟢 Easy (5 bom)", 7: "🟡 Medium (7 bom)", 12: "🔴 Hard (12 bom)", 17: "☠️ Expert (17 bom)"}
